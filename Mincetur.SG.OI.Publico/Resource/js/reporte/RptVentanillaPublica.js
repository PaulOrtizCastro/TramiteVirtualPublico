...
$(document).ready(function () {
    if (noEsVacio($("#idHfConsCodForm").val())) {
        $("#divButtonReturn").show();
    } else {
        $("#divButtonReturn").hide();
    }

    var ticket = new String();

    if (noEsVacio(idExpeVirtual)) {
        ticket = idExpeVirtual;
    } else {
        ticket = $("#idHfConsCodTicket").val().trim();
    }
    
    configuracionInicialRegExpe(ticket);
