...
basicos = {
    ...
    MultiplicaNumeros: function (ListaNumeros, decimal) {
        ...
        $.each(ListaNumeros, function (k, Numero) {
            ...
            if (posCaracterDecimal > 0) {
                ...
                numeroEntero = parseInt(parteEntera) * parseInt("1" + basicos.PadLeft("", parteDecima.toString().length, "0")) + parseInt(parteDecima);
        ...
        if (Producto.toString().length > cantDecimal) {
            ...
            resultado = "0." + basicos.PadLeft(Producto.toString(), dif, "0");
        }
        if (decimal != null) {
    ...
    Ajax: function (url, parameters, async, funcionSuccess) {
        ...
        $.ajax({
            ...
            dataType: "json",
            async: ((async && (typeof async != "undefined")) ? async : false),
    ...
    AjaxForm: function (options) {
        ...
        $.ajax({
            ...
            resetForm: defaults.resetForm,
            beforeSubmit: function (formData, jqForm, options) {
                return true;
            },
            success: function (response, textStatus, xhr) {
                var lstData = (typeof msg == "string") ? eval(response) : response;  
                rsp = lstData;     
                defaults.callBack(lstData, textStatus, xhr);
            },
            failure: function (msg) {
                alert(msg);
                rsp = msg;
    ...
    GetTimeToDate: function (date) {
        ...
        if (date != null && date != "") {
            if (date.indexOf("/Date(") >= 0) {
                ...
                var fecharecepcion = basicos.getDate(fecha) + '/' + basicos.getMonth(fecha) + '/' + fecha.getFullYear();
    ...
    Grilla: function (grilla, pager, identificador, height, width, caption, urlListar, id, colsNames, colsModel, sortName, opciones) {
        ...
        if (!opciones.GridLocal) {
                ...
                gridComplete: function () {
                    if ($('#' + grilla).getGridParam('records') == 0) {
                        if (opciones.noregistro == true) {
                            basicos.ShowAlert("dialog-alert", "Sin Registro");
                        }
                    }
            ...
            $('#' + grilla).jqGrid({
                ...
                groupingView: {
                    ...
                    groupText: ['<span style="background-image:none;background-color:#f4f3f3;display:block;margin-right:-2px;margin-left:-2px;padding:4px;font-weight:bold"> {0} - {1} Documento(s) </span>'],
                ...
                onClickButton: function () {
                    if (rowKey != null) {
                        if (opciones.btnEliminar == null) {
                            basicos.confirm(opciones.Mensaje, function () { Eliminar(); }, '', opciones.dialogDelete, opciones.Lenguaje);
                        }
                        else {
                            if (opciones.btnEliminar.Mensaje != null && opciones.btnEliminar.Mensaje != '') {
                                basicos.confirm(opciones.btnEliminar.Mensaje, function () { opciones.btnEliminar.Function(rowKey); }, '', opciones.btnEliminar.titleMensaje, opciones.Lenguaje);
                            }
                            else {
                        ...
                        if (opciones.Alert == null) {
                            basicos.Alert(opciones.dialogAlert, opciones.Mensaje, '', opciones.Lenguaje);
                        }
                        else {
                            basicos.Alert(opciones.Alert.titleMensaje, opciones.Alert.Mensaje, '', opciones.Lenguaje);
                        }
                    }
                }
            });
        ...
        var funccion = 'basicos.LayoutBarra("' + pager.toString() + '")';
    ...
    LoadDropDownListItems: function (name, url, parameters, selected) {
        ...
        combo.disabled = true;
        var items = "";
        var resultado = basicos.Ajax(url, parameters, false);
    ...
    CargarComboGrilla: function (url, parameters) {
        ...
        var cadena = "";
        var respuesta = basicos.Ajax(url, parameters, false);
        if (respuesta != null) {
    ...
    Alert: function (dialogTitle, dialogText, okFunc, idioma) {
        /*$('<div class="divAlertTramiteElect" style="padding: 10px; min-width: 250px; word-wrap: break-word;">' + dialogText + '</div>').dialog({
            draggable: false,
            modal: true,
            resizable: false,
            width: '400px',
    ...
    InicializarMsg: function () {
        $(".msgModel").show();
        basicos.msgHide();
        basicos.ShowElement(".msgModel .loading");
    },
    ...
    msgConfirm: function (rpt, Message, funcionout) {
        ...
        if (rpt) {
            $(".msgModel").slideUp(200, function () {
                ...
                basicos.ShowElement(".msgModel .mexito");
            ...
            $(".msgModel").slideUp(200, function () {
                ...
                basicos.ShowElement(".msgModel .merror");
            });
        }
        $(".msgModel").show();
